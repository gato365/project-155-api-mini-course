...
```{=html}
<iframe class="anim-embed" src="../animations/interactive/url_breakdown.html?embed=1" title="Interactive animation titled 'Anatomy of a GET Request' showing a URL being broken into components between a Client and API Server: protocol, host, endpoint path, and query string parameters" style="width:100%; height:640px; border:1.5px solid #c7d2fe; border-radius:14px; background:#f6f7ff;" loading="lazy"></iframe>
```
...
The two blanks that identify the *service* are `api` and `forecast`; the two that identify *where* are your latitude and longitude; the one that says *what* is `temperature_2m`. Tomorrow you will paste your paper URL into a real browser — keep it!
:::
