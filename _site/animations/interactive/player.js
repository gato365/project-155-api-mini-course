...
function initPlayer(opts) {
  ...
  // Embed mode: page.html?embed=1 hides the page chrome (title, home link)
  // and reports content height to the parent so the iframe can size itself
  if (/[?&]embed=/.test(window.location.search) && window.parent !== window) {
    document.body.classList.add('embed-mode');
    var postHeight = function () {
      window.parent.postMessage(
        { type: 'anim-embed-height', height: document.body.scrollHeight },
        '*'
      );
    };
